...
const CoinRow = ({ coin, isStarred, toggleWatchlist, toggleForm }) => {
	...
	const color = getColor(priceChange);
	const priceChangeLabel =
		Number.isFinite(priceChange) ? `${priceChange.toFixed(2)}%` : "--";
	const currentPriceLabel = Number.isFinite(currentPrice)
		? formatCurrency(currentPrice * currency[1], 6)
		: "--";
	const marketCapLabel = Number.isFinite(marketCap)
		? formatCurrency(marketCap * currency[1], 6)
		: "--";
	return (
